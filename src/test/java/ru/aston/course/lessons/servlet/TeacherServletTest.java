...
class TeacherServletTest {
    ...
    @Mock
    private TeacherJsonMapper jsonMapper;
    @Mock
    ...
    @BeforeEach
    void setup() {
        servlet = new TeacherServlet(service, jsonMapper);
    }
    ...
    void testDoGetWithNoIdParameter() throws Exception {
        ...
        when(service.findAll()).thenReturn(teacherDtos);
        when(jsonMapper.toJson(teacherDtos)).thenReturn(JSON_TEST);
        ...
        verify(jsonMapper, times(1)).toJson(teacherDtos);
    ...
    void testDoGetWhenEmptyList() throws Exception{
        ...
        when(request.getParameter(PARAMETER_ID)).thenReturn(null);
        when(service.findAll()).thenReturn(teacherDtos);
        when(jsonMapper.toJson(teacherDtos)).thenReturn("");
        when(response.getWriter()).thenReturn(mockWriter);
    ...
    void testDoPostWithNullIdWhenBodyPresent() throws Exception {
        ...
        TeacherDto dto = new TeacherDto();
        TeacherDto dtoSaved = new TeacherDto();
        PrintWriter mockWriter = mock(PrintWriter.class);
        dtoSaved.setId(1L);
        ...
        try(MockedStatic<ServletUtil> mockedStatic = mockStatic(ServletUtil.class)) {
            ...
            verify(service, times(1)).save(any(TeacherDto.class));
            verify(jsonMapper, times(1)).toJson(any(TeacherDto.class));
            verify(response, times(1)).setContentType(JSON_CONTENT_TYPE);
            verify(response, times(1)).setCharacterEncoding(JSON_CHARACTER_ENCODING);
    ...
    void testDoPostWithIdWhenBodyPresent() throws Exception {
        String jsonBody = "test";
        TeacherDto dto = new TeacherDto();
        TeacherDto dtoUpdate = new TeacherDto();
        PrintWriter mockWriter = mock(PrintWriter.class);
        dto.setId(1L);
        dtoUpdate.setId(1L);
        ...
        try(MockedStatic<ServletUtil> mockedStatic = mockStatic(ServletUtil.class)) {
            ...
            mockedStatic.verify(() -> ServletUtil.getJsonBody(request), times(1));
            verify(jsonMapper, times(1)).toDto(anyString());
            verify(service, times(1)).update(any(TeacherDto.class));
            verify(jsonMapper, times(1)).toJson(any(TeacherDto.class));
            verify(response, times(1)).setContentType(JSON_CONTENT_TYPE);
            verify(response, times(1)).setCharacterEncoding(JSON_CHARACTER_ENCODING);
            verify(response, times(1)).setStatus(HttpServletResponse.SC_OK);
